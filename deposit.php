...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($account_id > 0 && $amount > 0) {
        $query = "CALL deposit_amount($account_id, $amount)";
        if (mysqli_multi_query($conn, $query)) {
            ...
            $message = "₹$amount successfully deposited to Account ID $account_id.";
    ...
    <form method="POST">
        ...
        <select name="account_id" required>
            ...
            <?php while ($row = mysqli_fetch_assoc($accounts)) {
                echo "<option value='" . htmlspecialchars($row['account_id']) . "'>" . htmlspecialchars($row['account_id']) . "</option>";
            } ?>
        </select>
